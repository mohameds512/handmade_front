<template>
  <div class="auth-wrapper auth-v2">
    <b-row class="auth-inner m-0">
      ...
      <!-- Register-->
      <b-col cols="12" class="d-flex align-items-center auth-bg px-2 p-lg-5">
        <b-col sm="8" md="6" lg="8" class="px-xl-2 mx-auto">
          ...
          <validation-observer ref="registerForm" #default="{ invalid }">
            <b-form class="auth-register-form mt-2" @submit.prevent="register">
              ...
              <b-form-group label-for="password_confirmation" label="Password">
                <validation-provider #default="{ errors }" name="password_confirmation" vid="password_confirmation" rules="required">
                  <b-input-group class="input-group-merge" :class="errors.length > 0 ? 'is-invalid' : null">
                    <b-form-input id="password_confirmation" v-model="confirmPassword" class="form-control-merge"
                      :type="passwordFieldType" :state="confirmPasswordState" name="password_confirmation"
                      placeholder="············" />
                    <b-input-group-append is-text>
                      <feather-icon :icon="passwordToggleIcon" class="cursor-pointer" @click="togglePasswordVisibility" />
                    </b-input-group-append>
...
export default {
  ...
  computed: {
    confirmPasswordState() {
      if (this.passwordConfirmaion.length === 0) {
        return null;
      } else if (this.passwordConfirmaion === this.password) {
        return true;
      } else {
        return false;
      }
